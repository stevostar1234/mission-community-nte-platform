...
const quoteResolvedFormula = 'OR(NTE_Quote_Required_for_PO__c <> "Yes", NTE_Quote_Provided__c)';
const allInvoicesFormula = `AND(OR(NOT(${baseBillableFormula}), NTE_Invoice_Provided__c), OR(NOT(${topUpBillableFormula}), NTE_Top_Up_Invoice_Provided__c))`;
const allPaymentsFormula = `AND(OR(NOT(${baseBillableFormula}), NTE_Payment_Received__c), OR(NOT(${topUpBillableFormula}), NTE_Top_Up_Payment_Received__c))`;
const invoiceDueFormula = `OR(NOT(NTE_Pricing_Ready__c), AND(${quoteResolvedFormula}, OR(AND(${baseBillableFormula}, NOT(NTE_Invoice_Provided__c)), AND(${topUpBillableFormula}, NOT(NTE_Top_Up_Invoice_Provided__c)))))`;
const paymentDueFormula = `AND(NTE_Pricing_Ready__c, OR(AND(${baseBillableFormula}, NTE_Invoice_Provided__c, NOT(NTE_Payment_Received__c)), AND(OR(NOT(${baseBillableFormula}), NTE_Payment_Received__c), ${topUpBillableFormula}, NTE_Top_Up_Invoice_Provided__c, NOT(NTE_Top_Up_Payment_Received__c))))`;
const paymentConfirmedFormula = `AND(NTE_Pricing_Ready__c, OR(${baseBillableFormula}, ${topUpBillableFormula}), ${allInvoicesFormula}, ${allPaymentsFormula})`;
...
const leadFields = [
  ...
  picklist("NTE_Applicant_Email_Status__c", "NTE Applicant Email Status", ["Accepted by Salesforce", "Failed", "Not Attempted"]),
  ...
  ,currency("Listed_Price_Total__c", "Listed Price Total (ex VAT)")
...
const volunteerChoices = [
  ...
  "Charity Merchandise / Shop Support",
...
const volunteerLeadFields = [
  ...
  multipick("Volunteer_Opportunities__c", "Volunteering Opportunities", volunteerChoices, 6),
  multipick("Volunteer_Skills__c", "Volunteer Skills and Experience", volunteerChoices, 6),
...
const opportunityFields = [
  ...
  text("NTE_Invoice_Required__c", "NTE Invoice Required", 40),
  ...
  currency("NTE_Listed_Price_Total__c", "NTE Listed Price Total (ex VAT)"),
  ...
  formulaCheckbox("NTE_Finance_Invoice_Due__c", "NTE Finance Invoice Required", invoiceDueFormula, "Booking or staff top-up invoice work, including unresolved pricing."),
  formulaCheckbox("NTE_Finance_Payment_Due__c", "NTE Payment Required", paymentDueFormula, "An unpaid charge has its own invoice provided, following the booking-first payment order."),
  formulaCheckbox("NTE_Finance_Payment_Confirmed__c", "NTE Payment Confirmed", paymentConfirmedFormula, "Every applicable booking and staff top-up invoice and payment is complete. Fully complimentary bookings are excluded."),
  formulaCurrency(
    "NTE_Confirmed_Value__c",
    "NTE Confirmed Value (ex VAT)",
    "IF(NOT(NTE_Pricing_Ready__c), NULL, IF(AND(NOT(ISBLANK(Amount)), Amount > 0), Amount, NTE_Listed_Price_Total__c) + BLANKVALUE(NTE_Top_Up_Staff_Total__c, 0))",
    "Booking value excluding VAT, including staff top-ups. Bookings needing a pricing check are excluded."
  ),
  ...
  checkbox("NTE_Invoice_Provided__c", "NTE Invoice / Stripe Link Provided"),
  datetime("NTE_Invoice_Provided_At__c", "NTE Invoice / Stripe Link Provided At"),
  text("NTE_Invoice_Provided_By__c", "NTE Invoice / Stripe Link Provided By", 120),
  ...
  checkbox("NTE_Top_Up_Invoice_Required__c", "NTE Top-up Invoice Required"),
...
for (const field of opportunityFields) {
  ...
  if (field.api === "NTE_Invoice_Provided__c") field.description = "Selected from the NTE Master Panel after the invoice or Stripe payment link has been sent.";
...
const leadListViews = {
  ...
  NTE_Unmatched_Updates: ["NTE Unmatched Updates", ["FULL_NAME", "LEAD.COMPANY", "LEAD.EMAIL", "Web_Form_Type__c", "Target_Booking_Reference__c", "NTE_Update_Status__c", "NTE_Update_Error__c", "LEAD.CREATED_DATE"], [{field: "NTE_Update_Status__c", operation: "equals", value: "No Matching Booking,Ambiguous Booking Reference,Failed"}]],
...
Object.entries(leadListViews).forEach(([api, [label, columns, filters]]) => write(path.join(md, "objects", "Lead", "listViews", `${api}.listView-meta.xml`), listViewXml(api, label, columns, filters)));
...
const financeListColumns = ["OPPORTUNITY.NAME", "ACCOUNT.NAME", "NTE_Event_Code__c", "NTE_Listed_Price_Total__c", "NTE_Top_Up_Staff_Total__c", "NTE_Invoice_Provided__c", "NTE_Top_Up_Invoice_Provided__c", "NTE_Payment_Received__c", "NTE_Top_Up_Payment_Received__c", "NTE_Pricing_Status__c"];
const masterPanelOpportunityViews = {
  NTE_Quotes_Required: ["NTE Quotes Required", ["OPPORTUNITY.NAME", "ACCOUNT.NAME", "NTE_Event_Code__c", "NTE_Quote_Required_for_PO__c", "NTE_Quote_Provided__c", "NTE_Listed_Price_Total__c", "NTE_Pricing_Status__c"], [{field: "NTE_Quote_Required_for_PO__c", operation: "equals", value: "Yes"}, {field: "NTE_Quote_Provided__c", operation: "equals", value: "0"}]],
  NTE_Invoices_Required: ["NTE Invoice Required", financeListColumns, [{field: "NTE_Finance_Invoice_Due__c", operation: "equals", value: "1"}]],
...
const nteReports = {
  ...
  NTE_06_Finance_and_Invoicing: {
    ...
    description: "NTE finance work with pricing checks, quotations, booking and staff top-up charges, payment milestones and billing details. Confirmed value excludes VAT.",
    reportType: "Opportunity",
...
for (const reportApi of ["NTE_02_Partner_Sponsor_Event_Operations", "NTE_03_Exhibitor_Event_Operations", "NTE_06_Finance_and_Invoicing", "NTE_08_Event_Delivery_Readiness"]) {
  ...
  if (invoiceIndex >= 0) columns.splice(invoiceIndex + 1, 0, "Opportunity.NTE_Payment_Method__c");
}
...
}
{
  ...
  if (invoiceIndex >= 0) columns.splice(invoiceIndex + 1, 0, "Lead.Payment_Method__c");
}
...
for (const reportApi of ["NTE_06_Finance_and_Invoicing", "NTE_09_Weekly_Finance_Report"]) {
  ...
  report.columns.push("Opportunity.NTE_Pricing_Ready__c", "Opportunity.NTE_Finance_Invoice_Due__c", "Opportunity.NTE_Finance_Payment_Due__c", "Opportunity.NTE_Finance_Payment_Confirmed__c");
}
...
const templates = {
  ...
  NTE_Partner_Sponsor_Application_Acknowledgement: ["partner-sponsor-application-received.html", "NTE partnership / sponsorship application received", "NTE partner / sponsor application acknowledgement"],
  ...
  NTE_Exhibitor_Application_Acknowledgement: ["exhibitor-application-applicant-confirmation.html", "NTE exhibitor application received", "NTE exhibitor application acknowledgement"],
  ...
  NTE_Staff_Update_Acknowledgement: ["staff-update-applicant-confirmation.html", "Your NTE staff details", "NTE staff update acknowledgement"],
...
const emailPreviewValues = {
  ...
  Exhibitor_Space_Price__c: "800.00",
  ...
  Volunteer_Skills__c: "Driving / Transport; Mentoring & Coaching",
...
const repricedConversionFields = new Set([
  ...
  "Pricing_Version__c", "Invoice_Required__c"
]);
...
write(path.join(md, "flows", "NTE_Copy_Converted_Lead_to_Opportunity.flow-meta.xml"), [
  ...
  "    </actionCalls>",
  "    <actionCalls>",
  "        <name>Send_Exhibitor_Approval_Email</name>",
  "        <label>Queue NTE Provisional Email</label>",
  "        <locationX>176</locationX>",
  "        <locationY>396</locationY>",
  "        <actionName>NTEExhibitorApprovalEmailService</actionName>",
  "        <actionType>apex</actionType>",
  "        <connector><targetReference>Classify_Converted_Account</targetReference></connector>",
  "        <flowTransactionModel>CurrentTransaction</flowTransactionModel>",
  "        <inputParameters><name>leadId</name><value><elementReference>$Record.Id</elementReference></value></inputParameters>",
  "        <inputParameters><name>opportunityId</name><value><elementReference>$Record.ConvertedOpportunityId</elementReference></value></inputParameters>",
  "        <nameSegment>NTEExhibitorApprovalEmailService</nameSegment>",
  "    </actionCalls>",
  "    <apiVersion>67.0</apiVersion>",
  "    <areMetricsLoggedToDataCloud>false</areMetricsLoggedToDataCloud>",
  "    <description>Copies NTE application and event fields to the converted Opportunity and classifies the resulting Account and Contact for the NTE workspace. This avoids changing the org-wide LeadConvertSettings owned by another administrator.</description>",
  "    <environments>Default</environments>",
  ...
  "        <locationX>176</locationX>",
  "        <locationY>276</locationY>",
  "        <connector><targetReference>Send_Exhibitor_Approval_Email</targetReference></connector>",
...
function fieldInstanceXml(api, identifier, behavior = "none") {
  if (api === "NTE_Update_Booking_Id__c") behavior = "readonly";
  return [
    "        <itemInstances>",
    "            <fieldInstance>",
...
function recordPageXml({label, objectName, sections}) {
  ...
  sections.forEach((section, sectionIndex) => {
    ...
    const leftFields = section.fields.filter((_, index) => index % 2 === 0);
    const rightFields = section.fields.filter((_, index) => index % 2 === 1);
...
const leadSections = [
  ...
  {label: "Pricing", fields: ["Sponsor_Package_Total__c", "Exhibitor_Space_Price__c", "Power_Socket_Unit_Price__c", "Power_Socket_Total__c", "Additional_Staff_Unit_Price__c", "Additional_Staff_Total__c", "Top_Up_Staff_Unit_Price__c", "Top_Up_Staff_Total__c", "Listed_Price_Total__c", "Pricing_Status__c", "Pricing_Version__c"]},
  {label: "Invoicing and procurement", fields: ["Invoice_Required__c", "Payment_Method__c", "Invoiced_Company__c", "Trading_Name__c", "Invoice_Address__c", "Invoiced_Person__c", "Invoiced_Email__c", "Invoice_Contact_Phone__c", "Purchase_Order__c", "Purchase_Order_Number__c", "Quote_Required_for_PO__c", "Invoice_Additional_Information__c", "Supplier_Agreement_Required__c"]},
...
const opportunitySections = [
  ...
  {label: "Pricing", fields: ["NTE_Sponsor_Package_Total__c", "NTE_Exhibitor_Space_Price__c", "NTE_Power_Socket_Unit_Price__c", "NTE_Power_Socket_Total__c", "NTE_Additional_Staff_Unit_Price__c", "NTE_Additional_Staff_Total__c", "NTE_Top_Up_Staff_Unit_Price__c", "NTE_Top_Up_Staff_Total__c", "NTE_Listed_Price_Total__c", "NTE_Pricing_Status__c", "NTE_Pricing_Version__c"]},
  {label: "Invoicing and procurement", fields: ["NTE_Invoice_Required__c", "NTE_Payment_Method__c", "NTE_Invoiced_Company__c", "NTE_Trading_Name__c", "NTE_Invoice_Address__c", "NTE_Invoiced_Person__c", "NTE_Invoiced_Email__c", "NTE_Invoice_Contact_Phone__c", "NTE_Purchase_Order__c", "NTE_Purchase_Order_Number__c", "NTE_Quote_Required_for_PO__c", "NTE_Invoice_Additional_Information__c", "NTE_Supplier_Agreement_Required__c", "NTE_Quote_Provided__c", "NTE_Quote_Provided_At__c", "NTE_Quote_Provided_By__c", "NTE_Invoice_Provided__c", "NTE_Invoice_Provided_At__c", "NTE_Invoice_Provided_By__c", "NTE_Payment_Received__c", "NTE_Payment_Received_At__c", "NTE_Payment_Recorded_By__c", "NTE_Top_Up_Invoice_Required__c", "NTE_Top_Up_Invoice_Provided__c", "NTE_Top_Up_Invoice_At__c", "NTE_Top_Up_Invoice_By__c", "NTE_Top_Up_Payment_Received__c", "NTE_Top_Up_Payment_At__c", "NTE_Top_Up_Payment_By__c"]},
