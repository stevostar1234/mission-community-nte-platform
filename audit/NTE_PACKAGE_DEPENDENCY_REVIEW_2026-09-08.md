...
The new private repository is intended to contain all current source, tests, generators, configuration, plans, audit summaries/screenshots and the original client brief. Authentication caches, credentials, temporary snapshots and raw business-record exports are excluded. Historical `tmp/` evidence remains in the preserved local workspace. The publication record identifies the actual repository/commit and verification result once uploaded.
